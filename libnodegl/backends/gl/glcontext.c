...
struct glcontext *ngli_glcontext_new(const struct ngl_config *config)
{
    ...
    if (!glcontext)
        return NULL;
    glcontext->clazz = glcontext_class_map[glplatform];

    if (glcontext->clazz->priv_size) {
        glcontext->priv_data = ngli_calloc(1, glcontext->clazz->priv_size);
    ...
    if (glcontext->clazz->init) {
        int ret = glcontext->clazz->init(glcontext, config->display, config->window, config->handle);
        if (ret < 0)
            goto fail;
    }
...
{
    if (glcontext->clazz->make_current)
        return glcontext->clazz->make_current(glcontext, current);
...
{
    if (glcontext->clazz->set_swap_interval)
        return glcontext->clazz->set_swap_interval(glcontext, interval);
...
{
    if (glcontext->clazz->swap_buffers)
        glcontext->clazz->swap_buffers(glcontext);
}

void ngli_glcontext_set_surface_pts(struct glcontext *glcontext, double t)
{
    if (glcontext->clazz->set_surface_pts)
        glcontext->clazz->set_surface_pts(glcontext, t);
}
...
int ngli_glcontext_resize(struct glcontext *glcontext, int width, int height)
{
    ...
    if (glcontext->clazz->resize)
        return glcontext->clazz->resize(glcontext, width, height);
...
void ngli_glcontext_freep(struct glcontext **glcontextp)
{
    ...
    if (glcontext->clazz->uninit)
        glcontext->clazz->uninit(glcontext);
...
void *ngli_glcontext_get_proc_address(struct glcontext *glcontext, const char *name)
{
    ...
    if (glcontext->clazz->get_proc_address)
        ptr = glcontext->clazz->get_proc_address(glcontext, name);
...
void *ngli_glcontext_get_texture_cache(struct glcontext *glcontext)
{
    ...
    if (glcontext->clazz->get_texture_cache)
        texture_cache = glcontext->clazz->get_texture_cache(glcontext);
...
uintptr_t ngli_glcontext_get_display(struct glcontext *glcontext)
{
    ...
    if (glcontext->clazz->get_display)
        handle = glcontext->clazz->get_display(glcontext);
...
uintptr_t ngli_glcontext_get_handle(struct glcontext *glcontext)
{
    ...
    if (glcontext->clazz->get_handle)
        handle = glcontext->clazz->get_handle(glcontext);
...
GLuint ngli_glcontext_get_default_framebuffer(struct glcontext *glcontext)
{
    ...
    if (glcontext->clazz->get_default_framebuffer)
        fbo_id = glcontext->clazz->get_default_framebuffer(glcontext);
