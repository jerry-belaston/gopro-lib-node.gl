...
static int split_text(struct text *s, const char *str)
{
    ...
    for (size_t i = 0; i <= len; i++) {
        ...
        struct text_line line = {.buffer = buffer};
...
static int text_set_string_libs(struct text *s, const char *str)
{
    ...
    for (int i = 0; i < ngli_darray_count(&s->lines); i++) {
        ...
        for (int cp = 0; cp < len; cp++) {
            ...
            line_xmin = NGLI_MIN(line_xmin, chr.x + DF_CHAR_PAD);
            line_xmax = NGLI_MAX(line_xmax, chr.x + chr.w - DF_CHAR_PAD);
    ...
    for (int i = 0; i < nb_chars; i++) {
        ...
        // XXX: other aligns
        if (s->config.halign == NGLI_TEXT_HALIGN_CENTER) {
