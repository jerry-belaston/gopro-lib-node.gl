...
static int register_uniform(struct pass *s, const char *name, struct ngl_node *uniform, int stage)
{
    ...
    if (uniform->clazz->category == NGLI_NODE_CATEGORY_BUFFER) {
    ...
    } else if (uniform->clazz->category == NGLI_NODE_CATEGORY_UNIFORM) {
    ...
    if (params->properties) {
        ...
        if (resprops_node) {
            ...
            if (resprops->as_image) {
                if (texture->clazz->id != NGL_NODE_TEXTURE2D) {
