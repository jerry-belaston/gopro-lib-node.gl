...
static const struct node_param text_params[] = {
    ...
    {"font_file",    NGLI_PARAM_TYPE_STR, OFFSET(font_file),
                     .desc=NGLI_DOCSTRING("path to font file (require build with external text libraries)")},
    {"writing_mode", NGLI_PARAM_TYPE_SELECT, OFFSET(writing_mode), {.i64=NGLI_TEXT_WRITING_MODE_UNDEFINED},
...
static void destroy_characters_resources(struct text_priv *s)
{
    ...
    for (int i = 0; i < nb_descs; i++) {
        ...
        desc->glyph_rects_index = -1;
        desc->glyph_sizes_index = -1;
        desc->glyph_nb_index = -1;
        desc->modelview_matrix_index = -1;
        desc->projection_matrix_index = -1;
        desc->chr_transform_index = -1;
        desc->chr_color_index = -1;
        desc->chr_alpha_index = -1;
        desc->chr_stroke_width_index = -1;
        desc->chr_stroke_color_index = -1;
        desc->chr_glow_width_index = -1;
