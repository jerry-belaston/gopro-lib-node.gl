...
{
    if (node->clazz->id != NGL_NODE_ANIMATEDFLOAT &&
        node->clazz->id != NGL_NODE_ANIMATEDVEC2 &&
        node->clazz->id != NGL_NODE_ANIMATEDVEC3 &&
        node->clazz->id != NGL_NODE_ANIMATEDVEC4 &&
        node->clazz->id != NGL_NODE_ANIMATEDQUAT)
    ...
    if (node->clazz->id == NGL_NODE_ANIMATEDQUAT && s->as_mat4) {
    ...
    if (!s->anim_eval.kfs) {
        int ret = ngli_animation_init(&s->anim_eval, NULL,
                                      ...
                                      get_mix_func(node->clazz->id),
                                      get_cpy_func(node->clazz->id));
        if (ret < 0)
            return ret;
    }
    ...
    if (!kf0->function) {
        for (int i = 0; i < s->nb_animkf; i++) {
            int ret = s->animkf[i]->clazz->init(s->animkf[i]);
            if (ret < 0)
                return ret;
        }
    }
